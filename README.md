...
- `$XDG_STATE_HOME/llamactl/` for PID, launch metadata, generated swap config, logs, and profile benchmark results
...
llamactl profiles benchmark quality
llamactl profiles benchmarks quality
```
...
- [`STYLE.md`](STYLE.md) — TUI aesthetics and interaction conventions
- [`KEYBOARD.md`](KEYBOARD.md) — complete TUI keyboard controls
...
- Profile benchmarks run small (5%), medium (25%), and long (75%) uncached prompt-processing cases against the profile's effective context. Each case records PP throughput, average/peak/median decode throughput, first-response latency, memory peaks, and raw token arrival times. Only the latest two runs per profile are retained.
- The benchmark temporarily runs the selected profile directly with the selected llama.cpp runtime, then restores a previously running server or user service.
